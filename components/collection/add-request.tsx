...
const AddRequestModal = ({
	...
	const { data, isLoading, isError } = useCollections(selectedWorkspace?.id!);
	...
	const handleSubmit = async () => {
		...
		try{
			await mutateAsync({
				...
				name: requestName ?? requestData?.name,
			})
			toast.success(`Request saved to ${selectedCollection?.name} collection`)
			setIsModalOpen(false);
		}
		catch(err){
			console.error(`Failed to save request to ${selectedCollection?.name} collection`, err);
			toast.error(`Failed to save request to ${selectedCollection?.name} collection`);
		}
